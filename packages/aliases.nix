...
in
{
  programs.bash = {
    shellAliases = {
      ...
      sr = "cd ${homeDir}/nixos; git add . && export NIX_SSHOPTS='-p 6868' && nixos-rebuild switch --flake ${homeDir}/nixos#hetzner --target-host \"luka@91.99.69.65\" --use-remote-sudo; cd - &> /dev/null";
    };
    shellInit = ''
